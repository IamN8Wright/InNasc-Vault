...
function resendSettings() {
  const apiKey = process.env.INNASC_RESEND_API_KEY?.trim();
  const from = process.env.INNASC_EMAIL_FROM?.trim();
  if (!apiKey || !from) return null;
  return { apiKey, from, appUrl: process.env.INNASC_APP_URL?.trim() || 'http://localhost:3000' };
}
