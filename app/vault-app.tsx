...
function SettingsPage({ session, setSession, secure, showNotice, setError, signOut }: PageProps) {
  ...
  return <>
    ...
    <section className="settings-grid">
      ...
      <article className="panel setting-card"><div className="setting-title"><span className="stat-icon"><AlertTriangle /></span><div><h3>Offboarding export</h3><p>Portable client handoff</p></div><Badge variant="destructive">Plaintext secrets</Badge></div><p className="setting-copy">Downloads a clean, printable PDF containing the usernames, passwords, and other secret fields you are allowed to reveal and export. The PDF is not encrypted and must be secured immediately.</p><Alert><AlertTriangle /><AlertDescription>Requires fresh MFA and an explicit warning acknowledgment. Every export is recorded in the audit log.</AlertDescription></Alert><Button variant="destructive" onClick={exportOffboarding}><Download /> Export my offboarding PDF</Button></article>
