...
function Workspace({ session, setSession }: { session: Session; setSession: (session: Session | null) => void }) {
  ...
  const userManager = admin || session.user.role === 'client_admin';
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (userManager) requests.push(api<User[]>('/users'));
    ...
    <main className="vault-shell">
      <aside className="vault-sidebar">
        ...
        <nav aria-label="Primary navigation" className="vault-nav">
          <p>Workspace</p>
          {nav.filter((item) => item.page !== 'users' || userManager).map(({ page: itemPage, label, icon: Icon }) => (
...
function UsersPage({ data, session, refresh, secure, showNotice, setSession }: PageProps) {
  const clientAdmin = session.user.role === 'client_admin';
  const workspaceAdmin = session.user.role === 'workspace_owner' || session.user.role === 'admin';
    ...
    await secure(async () => {
      const created = await api<User & { welcomeEmail: { configured: boolean; sent: boolean } }>('/users', {
        ...
        body: JSON.stringify({
          ...
          canManage: form.get('canManage') === 'on',
          canReveal: form.get('canReveal') === 'on',
  ...
  async function removeUser(user: User) {
    if (!window.confirm(`Remove access for ${user.name}? Their sessions and MFA enrollment will be revoked, while audit history is preserved.`)) return;
    await secure(async () => {
      await api(`/users/${user.id}`, { method: 'DELETE', csrfToken: session.csrfToken });
      showNotice('User access removed and audited.');
      await refresh();
    });
  }
  ...
  async function permanentlyDeleteUser(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!deletingUser || deleteConfirmation.toLowerCase() !== deletingUser.email.toLowerCase()) return;
    const user = deletingUser;
    await secure(async () => {
      await api(`/users/${user.id}/permanent`, { method: 'DELETE', csrfToken: session.csrfToken, body: JSON.stringify({ confirmation: deleteConfirmation }) });
  ...
  async function resendWelcome(user: User) {
    const consequence = user.mustChangePassword
      ? 'This replaces their current temporary password immediately.'
      : 'This signs the user out, replaces their current password with a temporary password, and requires them to choose a new password after sign-in.';
    if (!window.confirm(`Send a welcome email to ${user.email}?\n\n${consequence}`)) return;
    await secure(async () => {
...
function SettingsPage({ session, setSession, secure, showNotice, setError, signOut }: PageProps) {
  ...
  async function exportOffboarding() {
    const acknowledged = window.confirm('This download will contain usernames, passwords, PINs, tokens, license keys, and secret notes in plaintext. It will not be encrypted. Store it securely and delete it when finished. Continue?');
    if (!acknowledged) return;
    await secure(async () => { await downloadFromApi('/exports/offboarding', { clientId: null, acknowledged: true }, session.csrfToken); showNotice('Confidential offboarding document downloaded. Secure it immediately.'); });
  }
  ...
  return <>
    ...
    <section className="settings-grid">
      ...
      <article className="panel setting-card"><div className="setting-title"><span className="stat-icon"><AlertTriangle /></span><div><h3>Offboarding export</h3><p>Portable client handoff</p></div><Badge variant="destructive">Plaintext secrets</Badge></div><p className="setting-copy">Downloads a clean, printable HTML document containing the usernames, passwords, and other secret fields you are allowed to reveal and export. The file is not encrypted and must be secured immediately.</p><Alert><AlertTriangle /><AlertDescription>Requires fresh MFA and an explicit warning acknowledgment. Every export is recorded in the audit log.</AlertDescription></Alert><Button variant="destructive" onClick={exportOffboarding}><Download /> Export my offboarding document</Button></article>
